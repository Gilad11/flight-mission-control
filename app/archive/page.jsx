...
import { Archive, Search, Trash2, RotateCcw, AlertTriangle, X, Calendar, Hotel, Users } from 'lucide-react';
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-500 dark:text-gray-400 mt-1 text-sm">נוסעים שהוסרו מטיסות פעילות</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 p-4 shadow-sm">
          <div className="text-2xl font-bold text-gray-900 dark:text-white">{new Set(travelers.map(t => t.flightId)).size}</div>
      ...
      <div className="flex flex-wrap gap-3 mb-5">
        <div className="relative flex-1 min-w-52">
          ...
          <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="חיפוש לפי שם, ת.ז, מ.א..."
        ...
        <select value={filterFlight} onChange={e => setFilterFlight(e.target.value)}
          ...
          {flights.map(f => <option key={f.id} value={f.id}>{f.flightNumber} - {formatDate(f.date)}</option>)}
        </select>
        ...
        {(search || filterFlight || filterHotel) && (
          <button onClick={() => { setSearch(''); setFilterFlight(''); setFilterHotel(''); }}
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
        <div className="px-4 py-3 border-b border-gray-100 dark:border-gray-800">
          <span className="text-sm text-gray-500 dark:text-gray-400">{filtered.length} תוצאות</span>
        </div>
        {loading ? (
                ...
                {filtered.map((t, idx) => {
                  ...
                  return (
                    <tr key={t.id} className={`border-t border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/40 transition-colors ${idx % 2 !== 0 ? 'bg-gray-50/30 dark:bg-gray-800/20' : ''}`}>
                      ...
                      <td className="px-4 py-3">
                        <span className={`text-xs px-2 py-1 rounded-full border font-medium ${colors.bg} ${colors.text} ${colors.border}`}>{t.group}</span>
                      </td>
                      <td className="px-4 py-3 text-sm">
                        {t.hotel ? (
                      ...
                      <td className="px-4 py-3 text-sm">
                        ...
                        ) : <span className="text-gray-400 text-xs">טיסה נמחקה</span>}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-center gap-2">
                          <button onClick={() => handleRestore(t.id)} className="p-2 text-green-600 hover:bg-green-50 dark:hover:bg-green-900/20 rounded-lg transition-all" title="שחזר נוסע">
                            <RotateCcw className="w-4 h-4" />
                          </button>
                          <button onClick={() => setConfirmDelete(t)} className="p-2 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-all" title="מחק לצמיתות">
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl p-6 w-full max-w-sm border border-gray-200 dark:border-gray-700">
            ...
            <div className="flex gap-2">
              <button onClick={() => handleDelete(confirmDelete.id)} className="flex-1 py-2.5 bg-red-600 hover:bg-red-700 text-white rounded-xl font-semibold text-sm transition-all">מחק לצמיתות</button>
              <button onClick={() => setConfirmDelete(null)} className="flex-1 py-2.5 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-xl font-semibold text-sm transition-all">ביטול</button>
            </div>
          </div>
        </div>
      )}
